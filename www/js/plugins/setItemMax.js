...
(function () {
	...
	/* added by nekoma following 8 lines. 2021.12.26 */
	(__makeSaveContents => {
		DataManager.makeSaveContents = function () {
			$gameSystem.villaA_itemIdMaxArray = villaA_itemIdMaxArray;
			$gameSystem.villaA_itemSeachNum = villaA_itemSeachNum;
			$gameSystem.villaA_itemMaxNum = villaA_itemMaxNum;
			return __makeSaveContents.apply(this, arguments);
		};
	})(DataManager.makeSaveContents);

	/* added by nekoma following 14 lines. 2021.12.26 */
	(__extractSaveContents => {
		DataManager.extractSaveContents = function (contents) {
			__extractSaveContents.apply(this, arguments);
			if ('villaA_itemIdMaxArray' in $gameSystem == false) {
				$gameSystem.villaA_itemIdMaxArray = [];
			}
			if ('villaA_itemSeachNum' in $gameSystem == false) {
				$gameSystem.villaA_itemSeachNum = 1;
			}
			if ('villaA_itemMaxNum' in $gameSystem == false) {
				$gameSystem.villaA_itemMaxNum = 2;
			}
			villaA_itemIdMaxArray = $gameSystem.villaA_itemIdMaxArray;
			villaA_itemSeachNum = $gameSystem.villaA_itemSeachNum;
			villaA_itemMaxNum = $gameSystem.villaA_itemMaxNum;
		};
	...
	Game_Interpreter.prototype.pluginCommand = function (command, args) {
		...
		if (command === 'アイテム上限設定変更') {
			for (var i = 0; i < args.length; i += 2) {
				/* 2023.02.25 modified upper 1 line to lower 6 lines by nekoma. */
				// villaA_itemIdMaxArray.push([args[i], args[i+1]]);
				let data = villaA_itemIdMaxArray.find(data => data[0] == args[i]);
				if (data) {
					data[1] = args[i + 1];
				} else {
					villaA_itemIdMaxArray.push([args[i], args[i + 1]]);
