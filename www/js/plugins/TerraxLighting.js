...
(function () {
	...
	var event_offsetX = [];	// added by nekoma from
	var event_offsetY = [];	// added by nekoma to
	...
	function ReloadMapEvents() {
		...
		event_offsetX = [];	// added by nekoma from
		event_offsetY = [];	// added by nekoma to
			...
			if ($gameMap.events()[i]) {
				if ($gameMap.events()[i].event()) {
					...
					if (note_command == "light" || note_command == "fire" || note_command == "flashlight" || note_command == "daynight") {
						let offset_x = 0;										// added by nekoma from
						let offset_y = 0;
						note = note.replace(/ ?([xy]):(-?\d+)/gi, (m, p1, p2) => {
							if (p1.match(/x/i)) {
								offset_x = Number(p2);
							} else {
								offset_y = Number(p2);
						...
						event_offsetX.push(offset_x / $gameMap.tileWidth());
						event_offsetY.push(offset_y / $gameMap.tileHeight());	// added by nekoma to
							...
							if (note_command == "light" || note_command == "fire" || note_command == "flashlight") {
								...
								if (light_radius >= 0) {
									...
									if (state == true) {
										...
										var ldir = 0;
										if (event_moving[i] > 0) {
											// lpx = $gameMap.events()[event_stacknumber[i]]._realX;					// modified by nekoma from
											// lpy = $gameMap.events()[event_stacknumber[i]]._realY;
											lpx = $gameMap.events()[event_stacknumber[i]]._realX + event_offsetX[i];
											lpy = $gameMap.events()[event_stacknumber[i]]._realY + event_offsetY[i];	// modified by nekoma to
											...
											// lpx = event_x[i];					// modified by nekoma from
											// lpy = event_y[i];
											lpx = event_x[i] + event_offsetX[i];
											lpy = event_y[i] + event_offsetY[i];	// modified by nekoma to
											ldir = event_dir[i];
										}
