...
(function () {
  ...
  //-----------------------------------------------------------------------------
  // added by nekoma(2021.09.06).
  //

  class Window_DescriptionEx2 extends Window_DescriptionEx {
    constructor() {
      super();
      this.x = 879;
      this.y = 63;
    ...
    drawEquipParameters(x, y, item) {
      ...
      for (var i = 0; i < 4; i++) {
        if (TextManager.param(i)) {
          this.changeTextColor(this.systemColor());
          this.drawText(TextManager.param(i), _x, _y, width);
          this.resetTextColor();;
          this.drawText(item.params[i], _x, _y, width, 'right');
          _y += lineHeight;
        }
      }
      _x = x + 193;
      ...
      this.drawPrice(x + 13, _y);
    }

    drawPrice(x, y) {
      if (TMPlugin.DescriptionEx.PriceText === '' || this._item.price === undefined) return y;
      this.changeTextColor(this.systemColor());
      this.drawText(TMPlugin.DescriptionEx.PriceText, x, y, 190);
      this.drawCurrencyValue((this._item.price * TMPlugin.DescriptionEx.PriceRate).toFixed(0),
        TextManager.currencyUnit, x, y, 190);
      return y + this.lineHeight();
    }

    drawTraits(x, y, item) {
      super.drawTraits(19, y + 8 + this.lineHeight() * 4, item);
    }
  ...
  (__createDescriptionExWindow => {
    Scene_Equip.prototype.createDescriptionExWindow = function () {
      __createDescriptionExWindow.apply(this, arguments);
      this.createDescriptionEx2Window();
    };
  })(Scene_Equip.prototype.createDescriptionExWindow);

  Scene_Equip.prototype.createDescriptionEx2Window = function () {
    this._descriptionEx2Window = new Window_DescriptionEx2();
    this.addWindow(this._descriptionEx2Window);
    this._slotWindow.setDescriptionEx2Window(this._descriptionEx2Window);
  };

  Window_EquipSlot.prototype.setDescriptionEx2Window = function (descriptionEx2Window) {
    this._descriptionEx2Window = descriptionEx2Window;
  };

  (__select => {
    Window_EquipSlot.prototype.select = function (index) {
      __select.apply(this, arguments);
      if (this._descriptionEx2Window) {
        let item = this.item();
        this._descriptionEx2Window.setItem(item);
      }
    };
  })(Window_EquipSlot.prototype.select);
