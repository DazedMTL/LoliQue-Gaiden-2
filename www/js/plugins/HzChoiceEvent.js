...
(function () {
    ...
    Game_Interpreter.prototype.updateWaitMode = function () {
        ...
        switch (this._waitMode) {
            case 'hzChoiceEvent':
                ...
                if (!waiting) {
                    // // 終了処理                              // modified by nekoma from
                    // this._hzChoceEvent.terminate();
                    // カーソルタイプはキャンセル時にのみ終了処理
                    if (this._hzChoceEvent._type == TYPE_LIST ||
                        this._hzChoceEvent._cancelCount == 0) {
                        this._hzChoceEvent.terminate();
    ...
    HzChoiceEvent.prototype.initialize = function (tag, resultVarNo, resetScroll, maxDist, type) {
        ...
        this._choiceEvents = $gameMap.events().filter(function (event) {
            ...
            // イベント画像先読み                                               // added by nekoma from
            if (hzChoice) {
            ...
            if (hzChoice == null) {
                return false;
            } else if (event._characterName === "") {   // added by nekoma from
                // 画像なしのイベントは選択対象からはずす
                return false;                           // added by nekoma to
    ...
    HzChoiceEvent.prototype.update = function () {
        ...
        // 初期選択イベントが「プレイヤーから接触」だったら実行 // added by nekoma from
        if ($gameVariables.value(this._resultVarNo) == 0) {
            if (this._selectedEvent._trigger == 1) {
                ok = true;
            }
        }                                                       // added by nekoma to
        ...
        if (this._type === TYPE_CURSOR) {
            ...
            if (TouchInput.isTriggered()) {
                ...
                // var events = $gameMap.eventsXy(x, y);    // modified by nekoma from
                var events = $gameMap.eventsXy(x, y)
                    .concat($gameMap.eventsXy(x - 1, y))
                    .concat($gameMap.eventsXy(x + 1, y));  // modified by nekoma to
                var self = this;
                var choiceEvents = events.filter(function (event) {
                    return self._choiceEvents.indexOf(event) >= 0;
                });
                if (choiceEvents.length > 0) {
                    if (this._selectedEvent !== choiceEvents[0]) {          // added by nekoma from
                        // 選択したイベントが「プレイヤーから接触」じゃなかったらスクロールテキストを閉じる
                        if (choiceEvents[0]._trigger != 1) {
                            SceneManager._scene._scrollTextWindow.close();
                        }
                        // マウス選択時にもカーソル音を鳴らす
                    ...
                    // var xrate = (TouchInput.x - Graphics.width  / 2) / Graphics.width;   // deleted by nekoma from
            ...
            if (left) {
                ...
                if (sortedEvents != null && sortedEvents.length > 0) {
                    ...
                    this.startScroll(this._selectedEvent, 10);
                    SoundManager.playCursor();
                    isCursorMoved = true;   // added by nekoma
                } else {
            ...
            if (isCursorMoved && this._selectedEvent) {             // added by nekoma from
                // 選択イベントが「プレイヤーから接触」だったら実行
                if (this._selectedEvent._trigger == 1) {
                    this.processOk();
                    return false;
                } else {
        ...
        // 選択イベントが変わっていなければ再描画しない         // added by nekoma from
        if (this._prevSelectedEvent == this._selectedEvent) {
            return true;
        } else {
            this._prevSelectedEvent = this._selectedEvent;
        }                                                       // added by nekoma to
    ...
    HzChoiceEvent.prototype.terminate = function () {
        var bitmap = SceneManager._scene._spriteset._hzChoiceScreen.bitmap;
        bitmap.clear();
        SceneManager._scene._scrollTextWindow.hide();   // added by nekoma
    };
