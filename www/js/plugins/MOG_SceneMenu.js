...
Scene_Menu.prototype.createCommands = function () {
	...
	for (var i = 0; i < this._comList.length; i++) {
		...
		this._compos[i] = [Moghunter.scMenu_ComX + (60 * i), Moghunter.scMenu_ComY + (0 * h)];
		this._comzoom[i] = 0;
...
Scene_Menu.prototype.commandMoveTo = function (value, real_value) {
	...
	var dnspeed = 10 + (Math.abs(value - real_value) / 20);
	if (value > real_value) {
		value -= dnspeed;
...
Window_MenuCommand.prototype.processCursorMove = function () {
	if (this.isCursorMovable()) {
		...
		if (this.index() !== lastIndex) {
			SoundManager.playCursor();
		} else if (isCursorMove) {									// added by nekoma from
			this._index = lastIndex == 0 ? this.maxItems() - 1 : 0;
		}															// added by nekoma to
	};
};
