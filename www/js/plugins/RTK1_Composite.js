 ...
 * @default
 * 
 * @param title window x
 * @desc
 * @default 0
 * 
 * @param title window y
 * @desc
 * @default 0
 * 
 * @param title window width
 * @desc
 * @default 0
 * 
 ...
 * @default
 * 
 * @param select window x
 * @desc
 * @default 0
 * 
 * @param select window y
 * @desc
 * @default 0
 * 
 * @param select window width
 * @desc
 * @default 0
 * 
 * @param select window height
 * @desc
 * @default 0
 * 
 * @param --- アイテム説明ウインドウ ---
 ...
 * @default
 * 
 * @param description window x
 * @desc
 * @default 0
 * 
 * @param description window y
 * @desc
 * @default 0
 * 
 * @param description window width
 * @desc
 * @default 0
 * 
 * @param description window height
 * @desc
 * @default 0
 * 
 * @param --- 実行ウインドウ ---
 ...
 * @default
 * 
 * @param execute window x
 * @desc
 * @default 0
 * 
 * @param execute window y
 * @desc
 * @default 0
 * 
 * @param execute window width
 * @desc
 * @default 0
 * 
 * @param execute window height
 * @desc
 * @default 0
 * 
 * @help
...
(function (_global) {
	...
	M._title_x = Number(param['title window x']) || 0;
	M._title_y = Number(param['title window y']) || 0;
	M._title_width = Number(param['title window width']) || 0;
	M._title_height = Number(param['title window height']) || 0;
	M._gold_x = Number(param['gold window x']) || 0;
	M._gold_y = Number(param['gold window y']) || 0;
	M._gold_width = Number(param['gold window width']) || 0;
	M._gold_height = Number(param['gold window height']) || 0;
	M._select_x = Number(param['select window x']) || 0;
	M._select_y = Number(param['select window y']) || 0;
	M._select_width = Number(param['select window width']) || 0;
	M._select_height = Number(param['select window height']) || 0;
	M._description_x = Number(param['description window x']) || 0;
	M._description_y = Number(param['description window y']) || 0;
	M._description_width = Number(param['description window width']) || 0;
	M._description_height = Number(param['description window height']) || 0;
	M._execute_x = Number(param['execute window x']) || 0;
	M._execute_y = Number(param['execute window y']) || 0;
	M._execute_width = Number(param['execute window width']) || 0;
	M._execute_height = Number(param['execute window height']) || 0;
	...
	M._window = M._window || {
		'title': {
			x: M._title_x,
			y: M._title_y,
			width: M._title_width,
			height: M._title_height
		},
		'gold': {
			x: M._gold_x,
			y: M._gold_y,
			width: M._gold_width,
			height: M._gold_height
		},
		'select': {
			x: M._select_x,
			y: M._select_y,
			width: M._select_width,
			height: M._select_height
		},
		'description': {
			x: M._description_x,
			y: M._description_y,
			width: M._description_width,
			height: M._description_height
		},
		'execute': {
			x: M._execute_x,
			y: M._execute_y,
			width: M._execute_width,
			height: M._execute_height
		}
	}
	...
	Scene_CompositeMenu.prototype.create = function () {
		...
		if (_chargeable()) {
			...
			var wx = M._window['gold'].x || Graphics.boxWidth - this._goldWindow.width;
			var wy = M._window['gold'].y || 0;
			var ww = M._window['gold'].width || this._goldWindow.width;
			var wh = M._window['gold'].height || this._goldWindow.height;
			// this._goldWindow.move(Graphics.boxWidth - this._goldWindow.width, 0, this._goldWindow.width, this._goldWindow.height);
			this._goldWindow.move(wx, wy, ww, wh);
			gw = this._goldWindow.width;
			this.addWindow(this._goldWindow);
		}

		this._helpWindow = new Window_Help(1);
		var wx = M._window['title'].x || 0;
		var wy = M._window['title'].y || 0;
		var ww = M._window['title'].width || Graphics.boxWidth - gw;
		var wh = M._window['title'].height || this._helpWindow.height;
		// this._helpWindow.move(0, 0, Graphics.boxWidth - gw, this._helpWindow.height);
		this._helpWindow.move(wx, wy, ww, wh);
		...
		var wx = M._window['select'].x || 0;
		var wy = M._window['select'].y || this._helpWindow.height;
		var ww = M._window['select'].width || 0;
		var wh = M._window['select'].height || 0;
		this._indexWindow = new Window_CompositeIndex(wx, wy, ww, wh, 2);
		...
		var wx = M._window['description'].x || 0;
		var wy = M._window['description'].y || wy;
		var ww = M._window['description'].width || ww;
		var wh = M._window['description'].height || wh;
		// this._statusWindow = new Window_CompositeStatus(0, wy, ww, wh);
		this._statusWindow = new Window_CompositeStatus(wx, wy, ww, wh);

		this._exeWindow = new Window_RTK_SingleCommand(0, 0, 1, [RTK.text("Execute")]);
		var wx = M._window['execute'].x || Graphics.boxWidth - 145 - this._exeWindow.textPadding();
		var wy = M._window['execute'].y || Graphics.boxHeight - this._exeWindow.height - this._exeWindow.textPadding();
		var ww = M._window['execute'].width || 145;
		var wh = M._window['execute'].height || this._exeWindow.height;
		...
		this._exeWindow.move(wx, wy, ww, wh);
	...
	Window_CompositeIndex.prototype.initialize = function (x, y, width, height, cols, rows) {
		x = x || 0;
		y = y || 0;
		width = width || Graphics.boxWidth;
		height = height || this.fittingHeight(rows || 6);
	...
	Window_CompositeStatus.prototype.refresh = function () {
		...
		var x = this.textPadding() + this._width / 3;
